...
void loop() {
  ...
  */
  ...
  if (cur_time - last_sent_time > 1000) {
    ...
    GET(get_builder("odinpalm-SW").c_str(), get_request,server_data.SW); 
    Serial.print("SW : ");
    Serial.println(server_data.SW);
      ...
      if (cur_buffer_length == expected_data_size) {
        switch (cur_data_header) {
          case UPDATE_PROJECT_DATA: {
              ...
              POST(set_builder("odinpalm-SW", SW).c_str(), update_data_to_server_callback);
            }
            break;
          case GET_SERVER_DATA:
